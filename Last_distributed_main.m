...
requestCount = [100:25:400];   % 每个时隙推理任务数量
J = 4;      % 固定服务器数量为5
K = 8;    % DL模型类型数量
T = 30;    % 时隙
frame = 1;    % 每帧所占时隙
...
for i=1:size(requestCount,2)%任务数变化结果
    for se = 1:size(seed_num, 2)
        ...
        disp(['随机算法完成']);
%         mine算法
        [X2,Y2,Z2,resultQoE2,resultProfit2,providerProfit2,placementCost2,updateCost2,runCost2,taskAccRate2,extime2] = Last_DMDTA_Algorithm(I,J,K,T,frame,F,K_It,serverCompuCap,storageCap,serverCompuCost, ...
